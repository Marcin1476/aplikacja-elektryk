...
# --- KONFIGURACJA STRONY ---
st.set_page_config(page_title="Asystent Elektryka Pro v2.3", layout="wide")
...
st.markdown("""
    ...
    .aparat-brand { font-size: 9px; font-weight: bold; letter-spacing: 1px; color: #fff; padding: 2px 0; text-transform: uppercase; }
    .aparat-faza { font-size: 13px; padding: 2px; font-weight: bold; margin-top: 5px; }
    ...
    for i, u in enumerate(rzad):
        ...
        <div class="aparat" style="width:{u.moduly*65}px; border-top: 15px solid {kolor_brandu};">
            ...
            <div class="aparat-body">
                <div class="aparat-header" style="font-size:9px; font-weight:bold; color:#555;">{u.nazwa}</div>
...
if st.session_state['szyna']:
    ...
    for r_idx, rzad in enumerate(rzedy):
        if rzad:
            st.write(f"**Szyna nr {r_idx + 1} ({wybrany_producent})**")
            dane = [{
                "Nr": f"A{i+1}",
                "Aparat": u.nazwa,
